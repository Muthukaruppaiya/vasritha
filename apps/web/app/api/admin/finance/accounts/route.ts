import { NextRequest } from "next/server";
import { fail, ok, requireAnyPermission, writeAuditLog } from "../../../../../lib/auth/api";
import { ensureFinanceSchema } from "../../../../../lib/finance";
import {
  createFinanceAccount,
  listFinanceAccounts
} from "../../../../../lib/finance-accounts";

export async function GET(request: NextRequest) {
  const { error } = await requireAnyPermission(request, [
    "finance:read",
    "reports:finance",
    "finance:write"
  ]);
  if (error) return error;
  await ensureFinanceSchema();
  const activeOnly = new URL(request.url).searchParams.get("active") === "1";
  return ok(await listFinanceAccounts({ activeOnly }));
}

export async function POST(request: NextRequest) {
  const { error, ctx } = await requireAnyPermission(request, ["finance:write"]);
  if (error || !ctx) return error;
  await ensureFinanceSchema();
  const body = (await request.json().catch(() => null)) as {
    code?: string;
    name?: string;
    account_type?: string;
    notes?: string;
  } | null;
  try {
    const row = await createFinanceAccount({
      code: body?.code || "",
      name: body?.name || "",
      account_type: body?.account_type || "",
      notes: body?.notes
    });
    await writeAuditLog({
      actorUserId: ctx.userId,
      action: "create",
      entityType: "finance_accounts",
      entityId: row.id,
      after: row
    });
    return ok(row, 201);
  } catch (err) {
    return fail(err instanceof Error ? err.message : "Failed to create account", 400);
  }
}
